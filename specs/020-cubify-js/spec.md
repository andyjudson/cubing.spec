# Feature 020 — cubify.js

## Summary

Design and build `cubify.js` — a focused, modern JavaScript library for 3×3 Rubik's cube visualisation, animation, and algorithm tooling. It replaces direct use of `cubing.js` / TwistyPlayer within this project and is architected to be reusable as a standalone open-source library.
...
**`cubify.js`** — consistent with the existing `cubify-scripts/` agent skill and `cubify-app/` utility. Not "twisty" anything.
...
- Lives in a new top-level directory `cubify-js/` within this repo initially
- Once stable, could be extracted to its own GitHub repository and published to npm
- `cfop-app` and `cubify-scripts` migrate to use `cubify.js` in place of `cubing.js`/TwistyPlayer in a subsequent feature
...
- **npm package** — publish as `cubify-js` once API is stable
